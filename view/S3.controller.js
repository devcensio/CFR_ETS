...
jQuery.sap.require("cfr.etsapp.manage.utils.Controller");
jQuery.sap.require("cfr.etsapp.manage.dev.devapp");
sap.ca.scfld.md.controller.BaseFullscreenController.extend("cfr.etsapp.manage.view.S3", {
	...
	prepareIDB: function() {
		if (window.indexedDB == null) {
			...
			createDBRequest.onsuccess = function(event) {
				...
				oDataStore.clear();
				var oTransaction = window.oController.myDB.transaction(["PersonnelAssigmentStore"], "readwrite");
				var oDataStore = oTransaction.objectStore("PersonnelAssigmentStore");
				oDataStore.clear();
				var oTransaction = window.oController.myDB.transaction(["FavoritesStore"], "readwrite");
				var oDataStore = oTransaction.objectStore("FavoritesStore");
				oDataStore.clear();
				...
				oDataStore.clear();
				var oTransaction = window.oController.myDB.transaction(["InitialInfoStore"], "readwrite");
				var oDataStore = oTransaction.objectStore("InitialInfoStore");
				oDataStore.clear();
				var oTransaction = window.oController.myDB.transaction(["TimeEntryStore"], "readwrite");
				var oDataStore = oTransaction.objectStore("TimeEntryStore");
				oDataStore.clear();
				var oTransaction = window.oController.myDB.transaction(["ValueHelpStore"], "readwrite");
				var oDataStore = oTransaction.objectStore("ValueHelpStore");
				oDataStore.clear();
				var oTransaction = window.oController.myDB.transaction(["WorklistStore"], "readwrite");
				var oDataStore = oTransaction.objectStore("WorklistStore");
				oDataStore.clear();
				var oTransaction = window.oController.myDB.transaction(["FavoriteDataFieldsStore"], "readwrite");
				var oDataStore = oTransaction.objectStore("FavoriteDataFieldsStore");
				oDataStore.clear();
			};
	...
	initializeView: function() {
		...
		this.setBtnText("SyncBtn", this.oApplicationFacade.getResourceBundle().getText("SYNC"));
		this.setBtnEnabled("SyncBtn", false);
	...
	submitTime: function(s) {
		...
		if (u.length !== 0) {
			for (i = 0; i < u.length; i++) {
				...
				e.push(a.setPostObject(u[i].entry.counter, "U", u[i].day, u[i].entry.time, u[i].entry.mainName, u[i].entry.mainCode, u[i].entry.notes,
					u[i].entry.startTime, u[i].entry.endTime, u[i].entry.subItems, u[i].entry.childCodes, u[i].entry.childNames));
			}
		}
		if (d.length !== 0) {
			for (i = 0; i < d.length; i++) {
				d[i].entry = this.replaceSpecialChar(d[i].entry);
				f.push(a.setPostObject(d[i].entry.counter, "D", d[i].day, d[i].entry.time, d[i].entry.mainName, d[i].entry.mainCode, d[i].entry.notes,
					d[i].entry.startTime, d[i].entry.endTime, d[i].entry.subItems, d[i].entry.childCodes, d[i].entry.childNames));
			}
		}
		...
		if (window.ftdf === 0) {
			...
			this.oService.getFavorites(this, this.oApplication.pernr, function(D) {
				for (var key in D) {
					...
					var oRecord = {
						...
						Field_Text: res.Field_Text,
						Name: res.Name,
						FavoriteDataFields: res.FavoriteDataFields,
						FavoriteOperation: res.FavoriteOperation
					};
					var oTransaction = window.oController.myDB.transaction(["FavoritesStore"], "readwrite");
					var oDataStore = oTransaction.objectStore("FavoritesStore");
					oDataStore.add(oRecord);
					var oRecord = {
						ID: res.ID,
						CATSHOURS: res.FavoriteDataFields.CATSHOURS,
						BEGUZ: res.FavoriteDataFields.BEGUZ,
						ENDUZ: res.FavoriteDataFields.ENDUZ
					};
					var oTransaction = window.oController.myDB.transaction(["FavoriteDataFieldsStore"], "readwrite");
					var oDataStore = oTransaction.objectStore("FavoriteDataFieldsStore");
					oDataStore.add(oRecord);
			...
			this.oService.getWorkListCollection(this, this.oApplication.pernr, this.getDateStr(f), this.getDateStr(l), function(D) {
				for (var key in D) {
					...
					var oRecord = {
						Pernr: res.Pernr,
						DataEntryProfileId: res.DataEntryProfileId,
						FieldName: res.FieldName,
						FieldText: res.FieldText,
						FieldValue: res.FieldValue,
						FieldValueText: res.FieldValueText,
	...
	setWeeklyData: function(d) {
		...
		for (i = 0; i < w.length; i++) {
			if (w[i].workingDay) {
				...
				if (!h) {
					a = new Date(parseInt(w[i].date.substring(0, 4), 10), parseInt(w[i].date.substring(4, 6), 10) - 1, parseInt(w[i].date.substring(6,
						8), 10));
					D = {
						date: a,
						dateStr: w[i].date,
	...
	getHeaderFooterOptions: function() {
		...
		var o = {
			...
			buttonList: [{
				...
				sI18nBtnTxt: "SYNC",
				onBtnPressed: function(e) {
					t.onSync(t, [], [], [], function(){alert("c'est passé youhou !");}, function(){alert("try again");});
				}
			},{
				sId: "copyBtn",
				sI18nBtnTxt: "Copy",
